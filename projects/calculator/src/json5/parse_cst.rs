...
fn parse_object(state: Input) -> Output {
    state.rule(Json5Rule::Object, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, "{", false)).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|s.sequence(|s|Ok(s).and_then(|s|parse_object_pair(s).and_then(|s| s.tag_node("object_pair"))).and_then(|s|builtin_ignore(s)).and_then(|s|s.repeat(0..4294967295, |s|s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, ",", false)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_object_pair(s).and_then(|s| s.tag_node("object_pair")))))).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|builtin_text(s, ",", false)))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, "}", false)))
    })
}
#[inline]
...
fn parse_object_key(state: Input) -> Output {
    state.rule(Json5Rule::ObjectKey, |s| {
        Err(s).or_else(|s|parse_integer(s).and_then(|s| s.tag_node("integer"))).or_else(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).or_else(|s|parse_string(s).and_then(|s| s.tag_node("string")))
    })
}
#[inline]
fn parse_array(state: Input) -> Output {
    state.rule(Json5Rule::Array, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, "[", false)).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|s.sequence(|s|Ok(s).and_then(|s|parse_value(s).and_then(|s| s.tag_node("value"))).and_then(|s|builtin_ignore(s)).and_then(|s|s.repeat(0..4294967295, |s|s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, ",", false)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_value(s).and_then(|s| s.tag_node("value")))))).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|builtin_text(s, ",", false)))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, "]", false)))
    })
}
#[inline]
fn parse_string(state: Input) -> Output {
    state.rule(Json5Rule::String, |s| {
        Err(s).or_else(|s|s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, "'", false)).and_then(|s|parse_string_raw(s).and_then(|s| s.tag_node("string_raw"))).and_then(|s|builtin_text(s, "'", false))).and_then(|s| s.tag_node("string_0"))).or_else(|s|s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, "'", false)).and_then(|s|s.repeat(0..4294967295, |s|parse_string_text(s).and_then(|s| s.tag_node("string_text")))).and_then(|s|builtin_text(s, "'", false))).and_then(|s| s.tag_node("string_1")))
    })
}
#[inline]
...
fn parse_string_text(state: Input) -> Output {
    state.rule(Json5Rule::StringText, |s| {
        Err(s).or_else(|s|parse_string_escape(s).and_then(|s| s.tag_node("string_escape"))).or_else(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^([^\"]+)").unwrap())}).and_then(|s| s.tag_node("string_text_1")))
    })
}
#[inline]
fn parse_string_escape(state: Input) -> Output {
    state.rule(Json5Rule::StringEscape, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, "\\", false)).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_any(s).and_then(|s| s.tag_node("c"))))
    })
}
#[inline]
